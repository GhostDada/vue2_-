  ...
  <div class="cart">
    ...
    <div class="cart-tool">
      <div class="select-all">
        <input class="chooseAll" type="checkbox" :checked="isAllCheck || cartInfoList > 0" @change="updateAllCheckedCart"/>
        <span>全选</span>
      </div>
      <div class="option">
...
import throttle from "lodash/throttle";
  export default {
    ...
    methods: {
      ...
      //处理某一个产品数量的个数(并且添加节流操作（规定时间内只能点击一次）)
      handler:throttle(async function(type,disNum,cart){
      //type类型   disNum产品数量变化量   cart 产品的信息(包含id和num)
        switch(type){
          //加号
          case'plus':
            disNum = 1;
        ...
        try {
          //返回成功
          await this.$store.dispatch('addOrUpdateCar',{skuId:cart.skuId,skuNum:disNum});
          //发送请求
          this.getData();
        } catch (error) {
    ...
    computed:{
      ...
      //是否全选
      isAllCheck(){
        //.every方法：item中isCheck全为 1 就返回真，有一个假就返回假
